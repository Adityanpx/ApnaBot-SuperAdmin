...
/**
 * Format paise as rupees with 2 decimal places
 * formatPaise(79) → "₹0.79"
 * formatPaise(150000) → "₹1,500.00"
 */
...
/**
 * Given rate cards already sorted by (countryCode, category, effectiveFrom desc)
 * — the order GET /api/admin/rate-cards returns — mark the id of the active
 * row per (countryCode, category) pair: the first row whose effectiveFrom is
 * <= today, since later rows for the same pair are older or not yet in effect.
 */
export function findActiveRateCardIds(rateCards) {
  const today = format(new Date(), 'yyyy-MM-dd');
  const seenPairs = new Set();
  const activeIds = new Set();

  for (const rc of rateCards) {
    const pairKey = `${rc.countryCode}|${rc.category}`;
    if (seenPairs.has(pairKey)) continue;
    if (rc.effectiveFrom > today) continue;
    seenPairs.add(pairKey);
    activeIds.add(rc._id);
  }
