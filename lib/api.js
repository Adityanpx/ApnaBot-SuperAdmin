...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (!refreshToken) {
        // No refresh token — hard logout. Goes through the store's logout()
        // (not the raw clearAuth() helper) so the middleware cookie is
        // cleared too — otherwise the cookie outlives localStorage and
        // middleware bounces /login back to /dashboard in a redirect loop.
        isRefreshing = false;
      ...
      try {
        ...
        // Refresh failed — clear auth (store logout, so the middleware
        // cookie is cleared alongside localStorage) and redirect to login
