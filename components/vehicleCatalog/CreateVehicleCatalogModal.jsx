// components/vehicleCatalog/CreateVehicleCatalogModal.jsx
'use client';
...
/**
 * CreateVehicleCatalogModal — form to add a new vehicle catalog entry
 * Props:
 *   open        boolean — is the modal visible?
 *   onClose     fn      — hide modal
 *   createEntry fn      — useVehicleCatalog().createEntry (toasts + refetches internally)
 *   uploadImage fn      — useVehicleCatalog().uploadImage
 */
export default function CreateVehicleCatalogModal({ open, onClose, createEntry, uploadImage }) {
  ...
  const resetForm = () => setForm({ name: '', type: '', seats: '', order: '', photoUrl: '' });
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      // error already toasted by createEntry — keep the modal open
    } finally {
      setLoading(false);
    }
  };
    ...
    <AnimatePresence>
      {open && (
        <Modal
          ...
          subtitle="Add a new vehicle type to the catalog"
          size="md"
        >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <input
                    ...
                    className="hidden"
                    id="vehicle-catalog-image-input"
                  />
                  <Button
                    type="button"
                    variant="secondary"
